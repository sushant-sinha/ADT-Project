...
public class FordFulkersonDFS {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            int[] result = fordFulkersonDFS(graph, source, sink);
            int paths = result[0];
    ...
    private static int[] fordFulkersonDFS(Map<Integer, Map<Integer, Integer>> graph, int source, int sink) {
        int paths = 0;
        int totalLength = 0;

        while (true) {
            List<Integer> augmentingPath = dijkstraDFSLike(graph, source, sink);
    ...
    private static List<Integer> dijkstraDFSLike(Map<Integer, Map<Integer, Integer>> graph, int source, int sink) {
        ...
        while (!minHeap.isEmpty()) {
            ...
            for (int v : graph.getOrDefault(u, Collections.emptyMap()).keySet()) {
                ...
                if (capacity > 0) {  // Check if the edge has positive capacity
                    int weight = 1; // Treat the edge lengths as unit distances for DFS-like
                    int alt = distance.get(u) + weight;

                    if (!distance.containsKey(v) || alt < distance.get(v)) {
                        distance.put(v, alt);
                        parent.put(v, u);
                        minHeap.offer(new Pair<>(v, decreasingCounter--)); // Use decreasing counter
                    }
                }
            }
        }
